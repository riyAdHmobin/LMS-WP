...
class Vibe_Cal_Settings{
    ...
    public static function init(){
        ...
        if ( is_null( self::$instance ) )
            self::$instance = new Vibe_Cal_Settings();
	...
	function initialize(){
		if(class_exists('VibeBP_Settings')){
			add_filter('vibebp_settings_tabs',function($tabs){
				$tabs['vibe_cal_function']=__('Calendar ','vibecal');
				return $tabs;
			});
		}else{
			add_options_page(__('Calendar ','vibecal'),__('Calendar ','vibecal'),'manage_options','vibecal',array($this,'settings'),1011);	
		}
		
    }
    
	function get_settings(){
		return apply_filters('vibe_cal_settings',array(
			array(
				'label' => __('Enable Google Event Create','vibecal'),
				'name' => 'google_calendar_googleevent_create',
				'type' => 'checkbox',
                'default'=> 0,
                'desc' => __('It will Google Event with Google Meet for event in event edit page.','vibecal'),
			),
			array(
				'label' => __('Google Project API Key','vibecal'),
				'name' => 'google_calendar_api_key',
				'type' => 'text',
                'default'=> '',
                'desc' => __('Get your Project API Key','vibecal').' <a href="https://console.developers.google.com/" target="_blank">help</a>',
            ),
            array(
				'label' => __('Google Project Client ID','vibecal'),
				'name' => 'google_calendar_client_id',
				'type' => 'text',
                'default'=> '',
                'desc' => __('Get your Project Client ID','vibecal').' <a href="https://console.developers.google.com/" target="_blank">help</a>',
			),
	...
	function settings(){
		...
		wp_nonce_field('vibe_cal_settings');   
		...
		echo '<tr valign="top"><th colspan="2"><input type="submit" name="save_vibe_cal_settings" class="button button-primary" value="'.__('Save Settings','vibecal').'" /></th>';
	...
	function generate_form($settings){
		...
		if(!empty($_GET['tab']) && $_GET['tab'] == 'vibe_cal_function'){
			$this->settings=get_option('vibe_cal_settings');
        }
			...
			switch($setting['type']){
				...
				case 'file':
						        ...
						        var selection = media_uploader2.state().get('selection');
						            ...
						            if(attachment &&  attachment.url !== undefined ){
						               pdf_url=attachment.url;
						            	...
						            	alert('<?php echo _x("Unable to find url of selected file","","vibe_cal");?>');
						            }
					...
					echo '<td class="forminp">
							<a class="upload_pdf_button button" data-uploader-button-text="'.__('Upload file','vibecal').'" data-input-name="'.$setting['name'].'" >'.__('Upload','vibecal').'</a>
				...
				case 'repeatable':
					...
					echo '<a class="wa_meta_box_repeatable_add button button-primary button-large" href="#">'.__('Add More','vibecal').'</a>
				...
				case 'page':
					echo '<th scope="row" class="titledesc"><label>'.$setting['label'].'</label></th>';
					echo '<td class="forminp"><select name="'.$setting['name'].'"><option>'.__('Select a page','vibecal').'</option>';
				...
				case 'instructor_field':
					...
					echo '<td class="forminp">';
					?>
					<a id="add_instructor_block" class="button"><?php _e('Add Searchable Instructor Field','vibecal'); ?></a>
						...
						jQuery(document).ready(function($){
							...
							var set_instructor_field = function(element){
								...
								element.find('.set_field').on('change',function(){
									...
									if(i === 'new' ){
										$(this).after('<div class="newoption"><input type="text" placeholder="<?php _e('Value','vibecal'); ?>" class="field_value"><input type="text" class="field_label " placeholder="<?php _e('Label','vibecal'); ?>"><a class="add_newoption button"><?php _e('Add','vibecal'); ?></a></div>');
							...
							if(instructor_blocks.length){
								instructor_blocks.map(function(item,i){
									if(item && item.hasOwnProperty('label')){
										...
										let block = '<li data-i="'+i+'"><span class="remove"></span><select class="set_field"><option value="none">'+item.label+'</option><option value="new"><?php _e('Add new option','vibecal'); ?></option>';
							...
							jQuery('#add_instructor_block').on('click',function(){
								...
								let appender = '<li data-i="'+instructor_blocks.length+'"><div style="display: grid; padding: 10px; text-align: center; grid-gap: 5px;"><input type="text" class="field_key" placeholder="<?php _e('Set Field key','vibecal'); ?>"><input type="text" class="field_name" placeholder="<?php _e('Set Field label','vibecal'); ?>"><a class="add_field_label button-primary"><?php _e('Add Field','vibecal'); ?></a></div><span class="remove"></span></li>';
								...
								jQuery('.add_field_label').on('click',function(){
									...
									if($(this).parent().find('input.field_name').val().length){
										...
										$(this).parent().after('<select class="set_field"><option value="none">'+$(this).parent().find('input.field_name').val()+'</option><option value="new"><?php _e('Add new option','vibecal'); ?></option></select>');
				...
				case 'directory_search':
					...
					echo '<td class="forminp">';
					?>
					<a id="add_search_block" class="button"><?php _e('Add Search block','vibecal'); ?></a>
					<ul id="search_blocks">
					</ul>
					<script>
						...
						jQuery(document).ready(function($){
							...
							$filters = array(
								array(
									'type'=>'course',
									'label'=> __('Search Course','vibecal'),
								),
								array(
									'type'=>'availability',
									'label'=> __('Select Times','vibecal'),
								),
								array(
									'type'=>'course-cat',
									'label'=> __('Select Category','vibecal'),
								),
								array(
									'type'=>'location',
									'label'=> __('Select Location','vibecal'),
								),
								array(
									'type'=>'level',
									'label'=> __('Select Levels','vibecal'),
								),
								array(
									'type'=>'text',
									'label'=> __('Select Text','vibecal'),
								),
							);
							...
							jQuery('#add_search_block').on('click',function(){
								...
								let options ='<option><?php _e('Select search option','vibecal'); ?></option>';
	...
	function save(){
		...
		if(!isset($_POST['save_vibe_cal_settings']))
			return;
		

		if ( !isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'],'vibe_cal_settings') ){
		     echo '<div class="error notice is-dismissible"><p>'.__('Security check Failed. Contact Administrator !','vibecal').'</p></div>';
		}
		...
		update_option('vibe_cal_settings',$this->settings);
		echo '<div class="updated notice is-dismissible"><p>'.__('Settings Saved.','vibecal').'</p></div>';
	}

}

Vibe_Cal_Settings::init();


function vibe_cal_function(){
	$init = Vibe_Cal_Settings::init();
	$init->settings();
}