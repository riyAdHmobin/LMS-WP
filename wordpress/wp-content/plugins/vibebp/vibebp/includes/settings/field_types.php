...
class VibeBP_Field_Types{
    ...
    function check_video_gallery_field($value,$user_id,$field){
    ...
    function check_video_gallery($value,$field){
        ...
        if($field->type=='gallery'){
            ...
            if(!empty($value)){
                ...
                $html .= '</div><style>.vibebp_media_gallery { display: flex; flex-direction: row; flex-wrap: wrap; justify-items: center; align-items: center; } .vibebp_media_gallery .vibebp_media_gallery_image{ width: 100px; margin: 1rem; position: relative;cursor:pointer }.vibebp_gallery_popup { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 999; display: flex; flex-direction: column; align-items: center; justify-content: center; }.vibebp_gallery_popup_content { width: 60vh; height: 90vh; display: flex; flex-direction: column; align-items: center; justify-content: center; overflow: hidden; }@media(max-width:500px){.vibebp_gallery_popup_content{width:80vw;}}</style>';
                ob_start();
                ?>
                <script>
    ...
    public function save_meta( $field ) {
        ...
        switch ( $field->type ) {
            ...
            case 'upload':
                ...
                $data = isset( $_POST['vibebp_upload_types'] ) ? wp_unslash( $_POST['vibebp_upload_types'] ) : '';
                bp_xprofile_update_field_meta( $field->id, 'vibebp_upload_types', $data );
                 $data = isset( $_POST['vibebp_upload_size'] ) ? wp_unslash( $_POST['vibebp_upload_size'] ) : '';
                bp_xprofile_update_field_meta( $field->id, 'vibebp_upload_size', $data );
                break;
            case 'video':
            $data = isset( $_POST['vibebp_video_types'] ) ? wp_unslash( $_POST['vibebp_video_types'] ) : '';
                bp_xprofile_update_field_meta( $field->id, 'vibebp_video_types', $data );
                 $data = isset( $_POST['vibebp_video_size'] ) ? wp_unslash( $_POST['vibebp_video_size'] ) : '';
                bp_xprofile_update_field_meta( $field->id, 'vibebp_video_size', $data );
              break;
            case 'gallery':
            $data = isset( $_POST['vibebp_gallery_types'] ) ? wp_unslash( $_POST['vibebp_gallery_types'] ) : '';
                bp_xprofile_update_field_meta( $field->id, 'vibebp_gallery_types', $data );
                 $data = isset( $_POST['vibebp_gallery_size'] ) ? wp_unslash( $_POST['vibebp_gallery_size'] ) : '';
